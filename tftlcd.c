 ...
 * @retval      无
 */
void lcd_show_chinese(uint16_t x, uint16_t y, char num, uint8_t size, uint16_t color)
{
    uint8_t temp, t1, t;
    uint16_t y0 = y;
    uint8_t *pfont = 0;
    ...
    {
        case 16:
            pfont = (uint8_t *)chinese_1616[num];  /* 调用1616字体 */
						fold = 2;
            break;
				case 40:
            pfont = (uint8_t *)chinese_4040[num];  /* 调用4040字体 */
						fold = 5;
            break;
    ...
    for (t = 0; t < size*fold; t++)
    {
        ...
        for (t1 = 0; t1 < 8; t1++)   /* 一个字节8个点 */
        {
            ...
            {
                LCD_Fast_DrawPoint(x, y, color);        /* 画点出来,要显示这个点 */
            }
            else if (mode == 0)     /* 无效点,不显示 */
            {
                LCD_Fast_DrawPoint(x, y, WHITE); /* 画背景色,相当于这个点不显示(注意背景色由全局变量控制) */
            }
